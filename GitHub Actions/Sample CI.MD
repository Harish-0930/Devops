...
| Secret Name     | Purpose                                                                        |
| --------------- | ------------------------------------------------------------------------------ |
| TOMCAT_URL      | Tomcat server URL (ex: [http://<IP>:8080](http://3.108.194.157:8080)) |
  ...
  full:
    ...
    steps:
      ...
      - name: Deploy WAR to Tomcat
        ...
        run: |
          WAR=$(ls target/*.war | head -n 1)
          curl -sS -u "${TOMCAT_USER}:${TOMCAT_PASS}" --upload-file "$WAR" "${TOMCAT_URL}/manager/text/deploy?path=/ecommerceapp&update=true"
```
